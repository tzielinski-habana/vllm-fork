...
from vllm.distributed.weight_transfer.torch_dist_transport import TorchDistCommunicator
...
def _make_communicator(process_group=None) -> TorchDistCommunicator:
...
class TestPlatformDispatch:
    ...
    def test_tcp_rendezvous_uses_torch_dist_off_cuda(
        ...
        monkeypatch.setattr(
            torch_dist_transport.TorchDistCommunicator, "create", fake_create
        )
...
class TestCreate:
    def test_joins_over_the_platform_backend(self, monkeypatch: pytest.MonkeyPatch):
        ...
        comm = TorchDistCommunicator.create("127.0.0.1", 12345, 1, 2, device=0)
        atexit.unregister(comm.destroy)
...
class TestDestroy:
    def test_aborts_and_unregisters_once_off_the_caller(
        ...
        comm = _make_communicator(process_group)
        comm.destroy()
    ...
    def test_does_not_wait_for_the_destructor(self, monkeypatch: pytest.MonkeyPatch):
        ...
        comm = _make_communicator(SlowToDestroy())
        comm.destroy()
