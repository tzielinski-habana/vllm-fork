...
def linear_batch_invariant(input, weight, bias=None):
    ...
    Two modes:
    - Inference (no grad): calls the best matmul kernel directly so that
      torch.compile/Inductor cannot replace the GEMM with a
      non-deterministic kernel. On XPU, uses the descriptor kernel.
    ...
    if input.device.type == "xpu":
        # On XPU the descriptor kernel expects B in [N, K] layout;
        # weight is already [out_features, in_features] = [N, K].
        batch_dims = input.shape[:-1]
        out_dim = weight.shape[0]
        input_2d = input.reshape(-1, input.shape[-1]).contiguous()
        result_2d = matmul_descriptor_persistent(input_2d, weight.t(), bias=bias)
        return result_2d.reshape(batch_dims + (out_dim,))
